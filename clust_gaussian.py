...
def AutoClass(data, numClusters):
    ...
    var_k = [[0.2] * numFeatures] * numClusters
    ...
    while abs((log_lhood - prev_lhood)/log_lhood)>epsilon:
        ...
        for n in range(len(data)):
            ...
            for k in range(numClusters):
                ...
                en_k[k] += gam[n][k]
                per_cluster.append(gam[n][k])
            per_data.append(per_cluster)
        gam = per_data

        # Maximization step
        for k in range(numClusters): 
            mu_k[k] = [0.0] * numFeatures
            var_k[k] = [0.0] * numFeatures
            for n in range(len(data)):
            ...
            for n in range(len(data)):
                for d in range(numFeatures):
                    temp_sum[k][d] += (gam[n][k]*(data[n][d]-mu_k[k][d])**2)
            for d in range(numFeatures):
            ...
            for k in range(numClusters):
                ...
                for d in range(numFeatures):
                    if var_k[k][d]==0:
                        ...
                        norm_prod *= 1.0/math.sqrt(2.0*math.pi*var_k[k][d])*math.exp(-(data[n][d]-mu_k[k][d])**2.0/(2.0*var_k[k][d]))
                    #print data[n][d]
                    #print mu_k[k][d]
                    #print var_k[k][d]
                    #print 1.0/math.sqrt(2.0*math.pi*var_k[k][d])*math.exp(-(data[n][d]-mu_k[k][d])**2.0/(2.0*var_k[k][d]))
                norm_sum += pi_k[k]*norm_prod
                print "k: " + str(k) + " & n: " +str(n) +" & " + str(pi_k[k]*norm_prod)
